...
class ZFBViewer(tk.Frame):
    ...
    def add_rom(self):
        ...
        size_options = ["640x480", "640x400", "144x208", "320x240"]
        selected_size = simpledialog.askstring("Image size", f"Select size ({', '.join(size_options)}):")
        if selected_size not in size_options:
            messagebox.showerror("Error", "Invalid or canceled size.")
            return
        ...
        try:
            if not os.path.exists(dest_rom_path):
                shutil.copy2(rom_path, dest_rom_path)
    
            with open(dest_rom_path, "rb") as f:
                data = f.read()
    
            if len(data) < img_data_size:
                messagebox.showerror("Error", "The ROM file does not contain enough data for that size.")
                return
    
            # Mapeo de extensiones según el core
            core_extensions = {
                "m2k": "zfb",
                "gba": "zgb", "dblcherrygb": "zgb", "gb": "zgb", "gbb": "zgb",
                "gbgb": "zgb", "gbgc": "zgb", "nes": "zfc", "nesq": "zfc", "snes": "zsf",
            ...
            # Obtener la extensión según el core
            extension = core_extensions.get(core, "zfb")
